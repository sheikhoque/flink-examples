...
public class StreamingJob {
	...
	private static final String DEFAULT_DELIVERY_STREAM_NAME = "TimestreamDeliveryStream";
	...
	public static void main(String[] args) throws Exception {
		...
		mappedInput.addSink(sink).uid("timestream-sink").name("timestreamsink");
		mappedInput.map(timestreamPoint -> gson.toJson(timestreamPoint)).addSink(createFirehoseSink(parameter)).uid("firehose-sink").name("firehosesink");
		// execute program
		env.execute("Flink Streaming Java API Skeleton");
	}

	private static SinkFunction<String> createFirehoseSink(ParameterTool parameter) {
		Properties outputProperties = new Properties();
		outputProperties.setProperty(AWSConfigConstants.AWS_REGION,
				parameter.get("Region", DEFAULT_REGION_NAME));
		return new FlinkKinesisFirehoseProducer<String>(DEFAULT_DELIVERY_STREAM_NAME, new SimpleStringSchema(), outputProperties);
	}
}
